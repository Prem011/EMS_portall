...
const EmployeesList = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await axios.get('/employees/employeeData');
        ...
        const employeeData = response.data.data.map(employee => {
          return {
              ...
              ? employee.image.replace(/^https?:\/\/(localhost:4001|ems-portall-5\.onrender\.com)\//, '') // Remove any unwanted prefix
  ...
  const handleDelete = async (employeeId) => {
    try {
      const response = await axios.post(`/employees/deleteEmployee/${employeeId}`);
